        ...
        while self.paused:
            try:
                ...
                if cmd in ('c', 'continue'):
                    ...
                    print(f"  当前突变率: {self.sim.population_manager.mutation_rate:.4f}")
                    if hasattr(self.sim.stagnation_manager, 'long_term_stagnation_counter'):
                        print(f"  全局停滞计数: {self.sim.stagnation_manager.long_term_stagnation_counter} / 10 (触发唤醒)")
                    if hasattr(self.sim.stagnation_manager, 'baie_stagnation_counter'):
